...
public class CameraDetector {
	...
	public class DirectionPosition {
		...
		public DirectionPosition(double[] edge, double[] x1y1, double[] x2y2, double direction) {
			this.x = edge[0];
			this.y = edge[1];
			this.x1y1 = x1y1;
			this.x2y2 = x2y2;
	...
	// Returns an angle value between 0 and 360
	public DirectionPosition getDirection(int robot, Mat frame) {
...
//				Imgproc.approxPolyDP(contour1, approxContour1, 3, true);
				...
				if (dirEdgePoint1 == null || dirEdgePoint2 == null) {
					System.out.println("dirEdgePoint1 or dirEdgePoint2 was null!");
					return null;
				}
				...
				return new DirectionPosition(directionEdge.get(0, 0), dirEdgePoint1.get(0, 0), dirEdgePoint2.get(0, 0), Math.atan2(directionEdge.get(0, 0)[1], directionEdge.get(0, 0)[0]));
			}
